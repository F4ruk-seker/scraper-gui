...
class MapScraper(object):
    ...
    def auto(self):
        ...
        try:
            ...
            self.scrape_comments(max_count=500)
        finally:
            self.close_bw()
    ...
    def change_arrangement(self, ar: ARM):
        ...
        if arl_count > AR.get_arrangement_count():
            logger.info(f"(change_arrangement): arrangement items count not true then ARRANGEMENT_COUNT > {arl_count} ")
        try:
    ...
    def get_comments_from_comment_bar(self, comment_bar, bar):
        ...
        #                               "/html/body/div[3]/div[9]/div[9]/div/div/div[1]/div[3]/div/div[1]/div/div/div[3]")
        comment_list = comment_bar.find_elements(By.XPATH, '//div[@jsaction and @data-review-id]')
        print(f"counter {self.__comment_counter} | comment list count : {len(comment_list)}")

        for comment_obj in comment_list[self.__comment_counter:]:
    ...
    @get_performance_metric_info_log
    def scrape_comments(self, max_count: int = 500, time_out: int = 0):
        declared: int = 0
        self.start_timer()
        ...
        def wrapper(self, comment_bar, declared, bar=None):
            while max_count > len(self.COMMENT_LIST):
                ...
                except BW_EXC.WebDriverException as wb_err:
                    logger.error("browser error from scrape_comments->wrapper")
                    logger.error(wb_err)
                    break
                except TimeoutException:
                    logger.info(f" scraper timeout, func had [{time_out}.seconds] | scraper taken {len(self.COMMENT_LIST)}data.element")
